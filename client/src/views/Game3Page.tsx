...
export default function Game3Page() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      socket.emit("guessRound3", {
        roomId,
        guesser: localStorage.username,
        guess: guesses,
      });
    } catch (error) {
      console.log(error);
  ...
  useEffect(() => {
    ...
    socket.on(
      "guessResult:server",
      (data: { guesser: string; isCorrect: boolean; submitter: string }) => {
        ...
        <div className="w-9/12 bg-white/10 p-6">
          <div className="bg-black bg-opacity-10 p-5 rounded-lg h-full flex flex-col justify-center items-center">
            ...
            <div className="flex-1 w-full flex flex-col items-center justify-center gap-4">
              ...
              />
              {drawingFromR2 ? null : <p className="text-white"></p>}
              <div>
                <input
                  type="text"
